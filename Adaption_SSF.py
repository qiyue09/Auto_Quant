...
zc.configure(broker_name='futures',  # 期货券商接口，目前仅'future'一个选项
             mode='realtrading',  # 模式选择，虚拟为'virtualtrading'，实盘为'realtrading'
             enter_license='s3az29vbx5w3',  # 登录通行证。暂设为此值即可
             account='202500101',  # 账号。虚拟盘中，为simnow投资者代码
             password='01123123',  # 密码。sinnow密码
...
zc.set_strategy('Adaption_SSF')  # 此处引入策略。注意：必须和交易策略py文件的命名严格一致
zc.run()
