...
class INFLECTION(Strategy):
    ...
    def generate_signal(self, dt: datetime):
        ...
        data['diff'] = diff_ema
        data['smooth'] = self.rngfilt(data['Close'], diff_ema)
        data['sma'] = data['Close'].rolling(window=self.parameters["sma_period"]).mean()
        ...
        if self.volatility == 0:
            ...
            self.high = self.volatility_center + (2 * data['diff'].iloc[-1])
            self.low = self.volatility_center - data['diff'].iloc[-1]
            if current_point > self.high:
                self.volatility = 0
                buy_signal = 1
            if current_point < self.low:
                self.volatility = 0
                sell_signal = 1
        elif self.volatility == -1:
            self.high = self.volatility_center + data['diff'].iloc[-1]
            self.low = self.volatility_center - (2 * data['diff'].iloc[-1])
            if current_point > self.high:
                self.volatility = 0
                buy_signal = 1
            if current_point < self.low:
        ...
        self.buy_signal = buy_signal*self.buy_signal + buy_signal
        self.sell_signal = sell_signal*self.sell_signal + sell_signal
        self.close_buy_signal = close_buy_signal*self.close_buy_signal + close_buy_signal
        self.close_sell_signal = close_sell_signal*self.close_sell_signal + close_sell_signal
        ...
        if not position:
            ...
            if self.buy_signal == 2 and self.volatility == 0:
            ...
            if self.sell_signal == 2 and self.volatility == 0:
            ...
            if self.close_buy_signal == 2:
                self.broker.relog()
                print(f'平多仓{self.instrument},profit:{profit},{current_point}')
                new_orders.append(close_buy_order)
                # self.write_order(instrument=self.instrument, type=3, point=current_point, profit=profit)
                if position['positionProfit'] <= 0:
                    self.volatility = 1
                    self.volatility_center = current_point
                    # self.high = current_point + (2 * data['diff'].iloc[-1])
                    # self.low = current_point - data['diff'].iloc[-1]
            ...
            profit = position['positionProfit'] / position['openPrice']
            if self.close_sell_signal == 2:
                print(f'平空仓{self.instrument}, profit:{profit},{current_point}')
                ...
                if position['positionProfit'] <= 0:
                    self.volatility = -1
                    self.volatility_center = current_point
                    # self.high = current_point + data['diff'].iloc[-1]
                    # self.low = current_point - (2 * data['diff'].iloc[-1])
