...
class INFLECTION(Strategy):
    ...
    def get_signal(self, df: pd.DataFrame, sma_period: int = 30, lookback: int = 5) -> int:
        """
        判断最近lookback根K线是否都在sma30上方或下方。

        参数：
            df: 包含至少 'close' 列的DataFrame
            sma_period: SMA的周期，默认为20
            lookback: 检查最近多少根K线，默认为5
        ...
        above = recent['Low'] > recent['sma']
        below = recent['High'] < recent['sma']
    ...
    def generate_signal(self, dt: datetime):
        ...
        close_buy_signal = 0
        close_sell_signal = 0
        sig = self.get_signal(data, self.parameters["sma_period"], self.parameters["lookback"])
        if data['smooth'].iloc[-1] > data['smooth'].iloc[-2]:
            ...
            if sig == -1:
                sell_signal = 1
        if data["Close"].iloc[-1] > data['sma'].iloc[-1]  and data["Open"].iloc[-1] > data['sma'].iloc[-1]:
            close_sell_signal = 1
        if data["Close"].iloc[-1] < data['sma'].iloc[-1]  and data["Open"].iloc[-1] < data['sma'].iloc[-1]:
            close_buy_signal = 1
