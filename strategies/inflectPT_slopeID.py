...
class INFLECTION(Strategy):
    ...
    def find_slope_zero(self, price, backtrace):
        """
        穷举 [2, max_bars) 区间长度 L，找到 “|斜率| 最小” 的 L。
        逻辑：在斜率最平（≈0）的一段做通道，更利于观察震荡。
        """
        price_ema = price.ewm(span=5, adjust=False).mean()
        max_slope = 1/backtrace
        zero_count = 0
        max_trace = 0

        for L in range(5, backtrace+1):
            y = price_ema.iloc[-L:]  # 最近 L 条
            x = np.arange(L).reshape(-1, 1)
            model = LinearRegression().fit(x, y.values)
            slope = model.coef_[0]
            if abs(slope) < max_slope:  # 更新最小斜率
                zero_count += 1
                max_trace = L
        return zero_count, max_trace
    ...
    def generate_signal(self, dt: datetime):
        ...
        if data['smooth'].iloc[-1] > data['smooth'].iloc[-2]:
            close_sell_signal = 1
            zero_count, max_trace = self.find_slope_zero(data['Close'], self.parameters["backtrace"])
            if zero_count < 2:
                buy_signal = 1
            elif zero_count >= 2:
                if data['Close'].iloc[-1] > data['Close'].iloc[-max_trace:].max():
                    buy_signal = 1
            ...
            if zero_count < 2:
                ...
                if data['Close'].iloc[-1] < data['Close'].iloc[-max_trace:].min():
                    sell_signal = 1
