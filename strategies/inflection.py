...
class potential_force(Strategy):
    ...
    def generate_signal(self, dt: datetime):
        ...
        if self.counter % self.retrain_interval == 0:
            ...
            df_feat['target'] = np.sign(df_feat['close'].shift(-3) - df_feat['close'])  # 明日涨跌

            X = df_feat[['force', 'potential']].iloc[:-4]
            y = df_feat['target'].iloc[:-4]
            self.model.fit(X, y)
        self.counter += 1
        # 用最新数据做预测
        recent_df = df.iloc[-(self.p.transformer_n + 1):]
        force_df = self.transformer.transform(recent_df)

        # 当前最新一行的特征（force 和 potential）
        current_row = force_df.iloc[-1]
        current_df = pd.DataFrame([current_row.values], columns=force_df.columns)

        # 新增：准确率判断逻辑

        pred = self.model.predict(current_df)
        signal = pred[0]
        print(signal)
